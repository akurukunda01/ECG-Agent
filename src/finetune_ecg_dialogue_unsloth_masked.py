...
Stress test 02 variant: identical to finetune_ecg_dialogue_unsloth.py except the loss is
computed on assistant-role tokens only (see the train_on_responses_only block in main).
"""
...
def main(model_name=None, max_seq_length=4096, output_dir=None, max_steps=None, eval_save_steps=1000, resume=False,
    ...
    # Sole delta vs the upstream script. Llama-3.2 role markers — not valid for Qwen/Hermes.
    ...
    _lab = list(_ex["labels"])
    assert _lab.count(-100) > 0, "masking produced no -100 labels"
    _graded = [t for t, l in zip(_ex["input_ids"], _lab) if l != -100]
    _dec = tokenizer.decode(_graded)
    print(f"[MASK CHECK] {_lab.count(-100)}/{len(_lab)} tokens masked; graded starts: {_dec[:80]!r}")
    assert "You will be provided with part of a dialogue" not in _dec, "system prompt leaked into graded tokens"
